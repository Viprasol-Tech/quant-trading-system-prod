...
export class HybridCompositeStrategy extends BaseStrategy {
  ...
  private calculateTrendScore(analysis: any): number {
    ...
    if (trend.ma_alignment === 'bullish') score += 40;
    else if (trend.ma_alignment === 'mixed') score += 20;

    if ((trend.ma_50_slope || trend.ma50_slope || 0) > 0) score += 20;
    if ((trend.ma_100_slope || trend.ma100_slope || 0) > 0) score += 20;
    if ((trend.ma_200_slope || trend.ma200_slope || 0) > 0) score += 20;
  ...
  private calculateMomentumScore(analysis: any): number {
    ...
    if ((momentum.macd || 0) > (momentum.macd_signal || 0)) score += 30;
    if ((momentum.macd_histogram || 0) > 0) score += 30;
  ...
  private calculateVolumeScore(analysis: any): number {
    ...
    if ((volume.obv_trend || 0) > 0) score += 30;
    if ((volume.mfi || 50) > 50) score += 20;

    return Math.min(100, score);
  }

  private calculatePatternScore(analysis: any, price: number): number {
    ...
    // Near support bonus
    const nearSupport = (sr.levels || []).some((l: any) =>
  ...
  private calculateRegimeScore(regime: string): number {
    switch (regime) {
