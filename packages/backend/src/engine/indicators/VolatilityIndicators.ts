...
export interface BollingerBands {
  ...
  lower: number;
  width: number;
  pctB: number; // Position within bands (0-1)
...
export class VolatilityIndicators {
   ...
   */
  static calculateATR(bars: OHLCV[], period: number = 14): number[] {
    if (period > bars.length) {
      logger.warn(`ATR period ${period} exceeds data length ${bars.length}`);
      return [];
    }
   ...
   */
  static calculateBollingerBands(closes: number[], period: number = 20, stdDev: number = 2): BollingerBands[] {
    if (period > closes.length) {
      logger.warn(`BB period ${period} exceeds data length ${closes.length}`);
      return [];
    }
   ...
   */
  static detectSqueeze(bands: BollingerBands[], threshold: number = 0.1): boolean {
   ...
   */
  static getVolatilityState(bars: OHLCV[], closes: number[], threshold: number = 0.1): VolatilityState {
   ...
   */
  private static getATRPercentile(atrValues: number[]): number {
    const validATR = atrValues.filter((v) => !isNaN(v));
    if (validATR.length < 2) return 0.5;
