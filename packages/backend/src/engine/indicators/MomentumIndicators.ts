...
export class MomentumIndicators {
  ...
  static calculateRSI(closes: number[], period: number = 14): number[] {
    if (period > closes.length) {
      logger.warn(`RSI period ${period} exceeds data length ${closes.length}`);
      return [];
    }
   ...
   * Interpret RSI value
   */
  static interpretRSI(rsi: number, overbought: number = 70, oversold: number = 30): RSIState {
    let signal: 'overbought' | 'oversold' | 'normal' = 'normal';
   ...
   */
  static detectBullishDivergence(
    bars: OHLCV[],
    indicator: number[],
    lookback: number = 30
  ): Divergence | null {
    ...
    if (
      priceLowest2.low < priceLowest1.low &&
      !isNaN(ind1) &&
      !isNaN(ind2) &&
      ind2 > ind1
    ) {
   ...
   */
  static detectBearishDivergence(
    bars: OHLCV[],
    indicator: number[],
    lookback: number = 30
  ): Divergence | null {
    ...
    if (
      priceHighest2.high > priceHighest1.high &&
      !isNaN(ind1) &&
      !isNaN(ind2) &&
      ind2 < ind1
    ) {
