...
export class VolumeIndicators {
   ...
   */
  static calculateRelativeVolume(volumes: number[], period: number = 20): number[] {
    if (period > volumes.length) {
      logger.warn(`RVOL period ${period} exceeds data length ${volumes.length}`);
      return [];
    }
   ...
   */
  static getOBVTrend(obv: number[], lookback: number = 10): 'accumulation' | 'distribution' | 'neutral' {
  ...
  static calculatePressure(
    bars: OHLCV[]
  ): { buyerPressure: number; sellerPressure: number }[] {
    const pressure: { buyerPressure: number; sellerPressure: number }[] = [];
   ...
   */
  static getVolumeConfirmation(
    bars: OHLCV[],
    priceDirection: 'up' | 'down' | 'neutral'
  ): string {
    if (bars.length < 2) return 'Insufficient data';
   ...
   */
  static calculateAverageVolume(bars: OHLCV[], period: number = 20): number {
