...
const strategies = {
    ...
    's2': new PullbackReversionStrategy_1.PullbackReversionStrategy(),
    's3': new HybridCompositeStrategy_1.HybridCompositeStrategy()
};
...
async function strategiesRoutes(fastify) {
    ...
    fastify.get('/api/strategies/:id', async (request, reply) => {
        ...
        const config = strategyConfigs.find(s => s.id === lowerKey ||
     ...
     */
    fastify.post('/api/strategies/:id/toggle', async (request, reply) => {
        const { id } = request.params;
        const lowerKey = id.toLowerCase();
        const configIndex = strategyConfigs.findIndex(s => s.id === lowerKey ||
            s.shortName.toLowerCase() === lowerKey);
        if (configIndex === -1) {
            return reply.status(404).send({
                success: false,
        ...
        logger_1.logger.info(`Strategy ${config.name} toggled to ${config.status}`);
        return reply.status(200).send({
            success: true,
            timestamp: new Date().toISOString(),
            data: {
