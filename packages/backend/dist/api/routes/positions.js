...
async function positionsRoutes(app) {
     ...
     */
    app.get('/api/positions/:symbol', async (request, reply) => {
        try {
            ...
            logger_1.logger.error('Position fetch failed:', error);
            return reply.status(500).send({
                success: false,
                error: {
     ...
     */
    app.post('/api/positions/:symbol/close', async (request, reply) => {
        try {
            const { symbol } = request.params;
            ...
            logger_1.logger.error('Position close failed:', error);
            return reply.status(500).send({
                success: false,
                error: {
                    code: 'CLOSE_FAILED',
