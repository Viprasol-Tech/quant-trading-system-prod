...
class TrendIndicators {
    ...
    static calculateSMA(prices, period) {
        if (period > prices.length) {
            logger_1.logger.warn(`SMA period ${period} exceeds data length ${prices.length}`);
            return [];
        }
        const sma = [];
     ...
     */
    static calculateMovingAverages(closes, periods = [50, 100, 200]) {
        return {
     ...
     */
    static getLatestMAs(closes, periods = [50, 100, 200]) {
     ...
     */
    static getTrendAlignment(closes, periods = [50, 100, 200]) {
     ...
     */
    static detectCrossovers(fastMA, slowMA, bars) {
        const crossovers = [];
        if (fastMA.length !== slowMA.length || fastMA.length !== bars.length) {
            logger_1.logger.warn('MA arrays have different lengths');
            return crossovers;
        }
     ...
     */
    static calculateMASlope(maValues, lookbackPeriod = 10) {
    ...
    static calculateEMA(prices, period) {
        if (period > prices.length) {
            logger_1.logger.warn(`EMA period ${period} exceeds data length ${prices.length}`);
            return [];
        }
        const ema = [];
     ...
     */
    static isUptrend(bars, lookbackPeriod = 20) {
     ...
     */
    static isDowntrend(bars, lookbackPeriod = 20) {
        if (bars.length < lookbackPeriod)
            return false;
        const recentBars = bars.slice(-lookbackPeriod);
        const highs = recentBars.map((b) => b.high);
...
exports.default = TrendIndicators;
//# sourceMappingURL=TrendIndicators.js.map