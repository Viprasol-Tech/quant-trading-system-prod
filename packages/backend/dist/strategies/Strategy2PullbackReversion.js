...
exports.Strategy2PullbackReversion = void 0;
 ...
 */
class Strategy2PullbackReversion {
    constructor() {
        this.name = 'Pullback/Mean Reversion';
        this.shortName = 'S2';
    }
    generateSignal(symbol, analysis, currentPrice) {
        ...
        const nearFibSupport = (currentPrice >= fib382 * 0.98 && currentPrice <= fib382 * 1.02) ||
            (currentPrice >= fib50 * 0.98 && currentPrice <= fib50 * 1.02);
        // Also check S/R support
        const supportLevels = analysis.support_resistance?.levels?.filter((l) => l.type === 'support') || [];
        const nearSupport = supportLevels.some((level) => currentPrice >= level.price * 0.98 &&
            currentPrice <= level.price * 1.02);
        if (!nearFibSupport && !nearSupport)
            return null;
        ...
        if (regime !== 'bull' && regime !== 'recovering')
            return null;
        ...
        const nearestSupport = supportLevels.length > 0
            ? Math.max(...supportLevels.map((l) => l.price).filter((p) => p < currentPrice))
            : currentPrice - (atr * atrMultiplier);
        const stopLossPrice = Math.min(nearestSupport * 0.99, // Just below support
        currentPrice - (atr * atrMultiplier));
        ...
        const confidence = Math.min(100, 45 +
            (nearFibSupport ? 15 : 0) +
            (nearSupport ? 10 : 0) +
...
exports.Strategy2PullbackReversion = Strategy2PullbackReversion;
exports.default = new Strategy2PullbackReversion();
//# sourceMappingURL=Strategy2PullbackReversion.js.map