...
exports.Strategy1TrendBreakout = void 0;
 ...
 */
class Strategy1TrendBreakout {
    constructor() {
        this.name = 'Trend Breakout Momentum';
        this.shortName = 'S1';
    }
    generateSignal(symbol, analysis, currentPrice) {
        ...
        const resistanceLevels = analysis.support_resistance?.levels?.filter((l) => l.type === 'resistance') || [];
        const nearResistance = resistanceLevels.some((level) => currentPrice >= level.price * 0.98 &&
            currentPrice <= level.price * 1.02);
        if (!nearResistance && resistanceLevels.length > 0)
            return null;
        ...
        if (!macdPositive || !macdRising)
            return null;
        // 6. Check regime - must be bull
        if (analysis.regime?.classification !== 'bull')
            return null;
        ...
        const confidence = Math.min(100, 50 +
        ...
        return {
            ...
            riskAmount: new decimal_js_1.default(riskPerShare * 100), // Assume 100 shares for calculation
...
exports.Strategy1TrendBreakout = Strategy1TrendBreakout;
exports.default = new Strategy1TrendBreakout();
//# sourceMappingURL=Strategy1TrendBreakout.js.map